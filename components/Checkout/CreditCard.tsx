...
const CreditCard = ({cardDetail}: {cardDetail: CreditCardContent[0]}) => {
  ...
  return (
    <div onClick={isSelected} className={`credit-card-wrapper ${selected ? 'credit-card-select': 'credit-card-default'} mt-3 mt-md-0 me-5`}>
      <div className='d-flex justify-content-between align-items-start'>
        <h3>{cardDetail.number}</h3>
        {selected? <FontAwesomeIcon icon={faCheckCircle} className='svg-icon-circle-check'/>:''}
      </div>
